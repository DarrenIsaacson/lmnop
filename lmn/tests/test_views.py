...
class TestAddNoteUnauthentictedUser(TestCase):
    fixtures = ['testing_artists', 'testing_venues',
                'testing_shows']  # Have to add artists and venues because of foreign key constrains in show
...
class TestAddNotesWhenUserLoggedIn(TestCase):
    ...
    def test_can_save_new_note_for_show_blank_data_is_error(self):
        ...
        self.assertEqual(Note.objects.count(),
                         initial_note_count)  # 2 test notes provided in fixture, should still be 2
...
class TestUserAuthentication(TestCase):
    ...
    def test_user_registration_logs_user_in(self):
        response = self.client.post(reverse('lmn:register'),
    ...
    def test_user_registration_redirects_to_correct_page(self):
        ...
        response = self.client.post(reverse('lmn:register'),
                                    {'username': 'sam12345', 'email': 'sam@sam.com', 'password1': 'feRpj4w4pso3az@1!2',
                                     'password2': 'feRpj4w4pso3az', 'first_name': 'sam', 'last_name': 'sam'},
                                    follow=True)

        self.assertRedirects(response, reverse(
            'lmn:homepage'))  # FIXME Fix code to redirect to last page user was on before registration.
